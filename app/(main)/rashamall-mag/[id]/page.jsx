...
const PostPage = () => {
  ...
  return (
    <div className="w-full mx-12 py-8 px-4 md:px-8">
      {/* Main Title */}
      <h1 className="text-xl md:text-3xl font-bold text-gray-800 mb-6">{post.mainTitle}</h1>
      ...
      {post.images && post.images.length > 0 && (
        <img src={post.images[0]} alt={post.mainTitle} className="w-full h-auto rounded-lg mb-8" />
      )}

      {/* Sections */}
      {post.sections.map((section) => (
        <div key={section._id} className="mb-6">
          <h2 className="text-lg md:text-xl font-semibold text-gray-800 mb-4">{section.title}</h2>
